...
// ║     Powered by Ourin Baileys + Node.js                   ║
  ...
  broadcastInterval = setInterval(async () => {
    ...
    for (const bc of pending) {
      try {
        ...
        // Saluran (@newsletter) dan grup: Ourin Baileys memakai sendMessage; relayMessage mengode plaintext untuk newsletter.
        ...
        if (hasMedia) {
          ...
          try {
            ...
            if (isNewsletter) {
              // Fallback ke teks saja — bug kompatibilitas media newsletter pakai CDN path berbeda (/o1/ vs /m1/)
...
async function startBot() {
  ...
  // Gunakan versi WhatsApp Web terbaru jika endpoint tersedia. Jika lookup gagal,
  // biarkan Ourin Baileys memakai versi bawaan paket yang sudah teruji.
  const versionInfo = await fetchLatestWaWebVersion();
  if (versionInfo.isLatest) {
    logger.info('VERSION', `Ourin Baileys / WA Web v${versionInfo.version.join('.')}`, '(latest)');
  } else {
    logger.warn('VERSION', 'Lookup versi WA Web gagal; menggunakan versi bawaan Ourin Baileys');
  }

  // Create WA Socket
  const sock = makeWASocket({
    ...(versionInfo.isLatest ? { version: versionInfo.version } : {}),
    logger: pinoLogger,
